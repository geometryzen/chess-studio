...
export const filepath = electron.app
    ? path.join(electron.app.getPath("userData"), filename) // in Main process
    : path.join(querystring.parse(global.location.search.slice(1))["user_data_path"] as string, filename); // in Renderer process
...
export class Config {
    ...
    book_explorer: boolean;
    lichess_explorer: boolean;
}
...
export const load = (): [string, Config] => {
    let cfg = new Config();

    let err_to_return: string = null;
    ...
    try {
        if (fs.existsSync(filepath)) {
            ...
            try {
                ...
                console.log(filename, err.toString(), "...trying to debork...");

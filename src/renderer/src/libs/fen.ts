...
export function LoadFEN(fen: string): Position {
    ...
    if (ret.attacked(opponent_king_square, ret.color(opponent_king_square))) {
...
function CastlingRights(board: Position, s: string): string {
    // s is the castling string from a FEN
    ...
    if (wk_location) {
        for (let ch of s) {
            if (["A", "B", "C", "D", "E", "F", "G", "H"].includes(ch)) {
                let point = point_from_s(ch.toLowerCase() + "1");
                if (board.piece(point) === "R") {
                    dict[ch] = true;
                }
            }
            if (ch === "Q") {
                if (board.state[0][7] === "R") {
                    // Compatibility with regular Chess FEN.
                    dict.A = true;
                } else {
...
function EnPassantSquare(board: Position, s: string) {
    // board.active must be correct. s is the en-passant string from a FEN.

    // Suffers from the same subtleties as the enpassant setter in position.move(), see there for comments.

    let p = point_from_s(s.toLowerCase());
    ...
    if (board.active === "w") {
        ...
        if (board.piece(point_from_xy(p.x, 3)) !== "p") {
            return null;
        }
        // Check the capture square is actually empty...
        if (board.piece(point_from_xy(p.x, 2)) !== "") {
            return null;
        }
        // Check potential capturer exists...
        if (board.piece(point_from_xy(p.x - 1, 3)) !== "P" && board.piece(point_from_xy(p.x + 1, 3)) !== "P") {
            return null;
        }
        return p;
    }
    ...
    if (board.active === "b") {
        ...
        // Check the takeable pawn exists...
        if (board.piece(point_from_xy(p.x, 4)) !== "P") {
            return null;
        }
        // Check the capture square is actually empty...
        if (board.piece(point_from_xy(p.x, 5)) !== "") {
            return null;
        }
        // Check potential capturer exists...
        if (board.piece(point_from_xy(p.x - 1, 4)) !== "p" && board.piece(point_from_xy(p.x + 1, 4)) !== "p") {
            return null;
        }
        return p;
    }
...
function IsNormalChessPosition(board: Position): boolean {

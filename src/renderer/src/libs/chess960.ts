...
function c960_arrangement(n: number): string {
    ...
    let insert = (i: number, piece: string) => {
        for (let n = 0; n < 8; n++) {
            if (pieces[n] === "." && --i < 0) {
                // Careful! Remember short-circuit rules etc.
                pieces[n] = piece;
                return;
            }
        }
...
export function c960_fen(n: number): string {

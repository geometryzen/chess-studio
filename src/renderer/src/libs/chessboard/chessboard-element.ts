...
export class ChessBoardElement extends LitElement {
    ...
    render() {
        return html`
            <div part="spare-pieces">${this._renderSparePieces(this.orientation === "white" ? "black" : "white")}</div>
            ${this._renderBoard()}
            <div part="spare-pieces">${this._renderSparePieces(this.orientation === "white" ? "white" : "black")}</div>
            <div
                id="dragged-pieces"
                style=${styleMap({
    ...
    private _renderSparePieces(color: SquareColor) {
        ...
        return html`
            <div></div>
            ${pieces.map((p) => html` <div id="spare-${p}" @mousedown=${this._mousedownSparePiece} @touchstart=${this._mousedownSparePiece}>${this._renderPiece(p, {}, false, sparePieceId(p))}</div> `)}
            <div></div>
        `;
    }
    ...
    private _renderBoard() {
        ...
        for (let y = 0; y < 8; y++) {
            for (let x = 0; x < 8; x++) {
                const file = COLUMNS[isFlipped ? 7 - x : x];
                const rank = isFlipped ? y + 1 : 8 - y;
                ...
                squares.push(html`
                    ...
                    >
                        ${this.showNotation && y === 7 ? html`<div part="notation alpha">${file}</div>` : nothing} ${this.showNotation && x === 0 ? html`<div part="notation numeric">${rank}</div>` : nothing}
    ...
    _renderPiece(piece: Piece | undefined, styles: Partial<CSSStyleDeclaration>, isDragSource?: boolean, id?: string, part?: string) {
        ...
        return html` <div id=${ifDefined(id)} part="piece ${part ?? ""}" piece=${piece} style=${styleMap(style as StyleInfo)} ...=${renderPieceDirective(piece, this.renderPiece)}></div> `;
    }
    ...
    private _isXYOnSquare(x: number, y: number): Location | "offboard" {
        ...
        const elements = (this.shadowRoot as unknown as Document)!.elementsFromPoint(x, y);
